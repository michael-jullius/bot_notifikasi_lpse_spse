...
def user_log(id, id_user, nama, massage):
    sql = "SELECT id FROM `user_log_pu` WHERE id_user = %s ORDER BY id DESC LIMIT 1"
    adr = (id_user,)
    cursor.execute(sql, adr)
    id_chat = cursor.fetchall()
    if id_chat == []:
        sql = "INSERT INTO `user_log_pu` (`id`, `id_user`, `nama`, `massage`) VALUES (%s, %s, %s, %s)"
...
def register(id_user, nama):
    sql = "SELECT id_user FROM `user_pu` WHERE id_user = %s"
    adr = (id_user,)
    cursor.execute(sql, adr)
